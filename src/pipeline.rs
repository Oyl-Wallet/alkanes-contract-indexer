...
impl Pipeline {
		...
		P: DeezelProvider + JsonRpcProvider + BitcoinRpcProvider + Send + Sync,
	{
		...
		if !op_return_txs.is_empty() {
			...
			// Build inputs for PoolSwap and PoolCreation indexers and run them
			...
			for (tx_index, r) in results.iter().enumerate() {
				...
				creation_inputs.push((r.transaction_id.clone(), tx_index as i32, ts, r.transaction_json.clone(), trace_events_json));
			}
